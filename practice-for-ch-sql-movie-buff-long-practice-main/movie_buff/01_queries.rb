...
def harrison_ford
  ...
  Movie.joins(:actors).where("actors.name = 'Harrison Ford'").where.not("castings.ord = 1").select('movies.id, movies.title')
end
...
def biggest_cast
  ...
  Movie
    .joins(:castings)
    .group('title', 'id')
    # .having("COUNT(?) AS num_actors", "castings.actor_id")
    # .count("castings.actor_id AS num_actors")
